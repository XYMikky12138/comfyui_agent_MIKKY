...
def _load_models_registry() -> List[Dict]:
    ...
    candidates = [
        plugin_root.parent / "ComfyUI_RH_OpenAPI" / "models_registry.json",
        plugin_root / "models_registry.json",
    ]
...
class RHAgentNode:
    ...
    def generate(
        ...
        **kwargs,
        ...
        parsed_prompts = self._parse_prompts(
    ...
    def _parse_prompts(
        ...
        if enable_validation and vlm_key:
            ...
            try:
                ...
                # 将参考图转为 base64 传给验证器，使其能识别主体特征
                ref_image_b64 = validator._tensor_to_base64(image_tensors[0]) if image_tensors else None
                if ref_image_b64:
                    print("  [OK] 参考图已传入 VLM 验证器")
                result = validator.validate_json(cleaned, ref_image_b64=ref_image_b64)
                if result.get("valid"):
        ...
        if result.get("valid"):
            return result.get("parsed_data", {}).get("prompts", [])
        raise ValueError(
