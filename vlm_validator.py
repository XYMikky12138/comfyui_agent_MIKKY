...
**禁止**将参考图中的当前状态转化为字面描述写入提示词。原因：字面描述（如"wearing red dress"、"short black hair"）会导致每次生成结果不一致，AI 对文字的理解会带来偏差。

**正确做法**：始终用"与参考图保持一致"的引用句式替代状态描述。

| 错误写法（禁止）| 正确写法（必须）|
|---|---|
| "wearing a white T-shirt and black pants" | "with the same clothing as in the reference image" |
| "short black hair, oval face" | "maintaining the same hairstyle and facial features as in the reference image" |
| "slim build, medium height" | "with the same body proportions as shown in the reference image" |
| "red sneakers, silver watch" | "keeping all accessories identical to the reference image" |

### 引用句式模板

描述外观/服装/体型时，使用以下句式（可组合）：
- `consistent with the person in the reference image`
- `maintaining the same [clothing / hairstyle / appearance] as shown in the reference image`
- `with identical [features / outfit / look] to the reference image`
- `keeping all visual characteristics of the subject unchanged from the reference`
- 多图时：`consistent with image 1 / as shown in image 2`

### 允许写入提示词的内容

以下内容**可以**直接写入提示词（因为是用户指令要求，而非当前状态）：
- 动作/姿势（用户明确要求的，如 "walking", "sitting"）
- 场景/背景（用户明确要求的，如 "outdoor park", "white studio background"）
- 视角（如 "front view", "side view"）
- 光线/风格（如 "natural light", "cinematic"）
- 技术参数（如 "1:1 aspect ratio", "high resolution"）

## 性别保留规则（铁律）

- 参考图中可见性别 → 以参考图为准，使用引用句式保留（如 `same gender as in reference image`）
- 原始提示词中有明确性别词（female/male/woman/man/女/男）且与参考图一致 → 保留该性别词
...
class VLMValidator:
    ...
    def _convert_natural_language(self, text: str, ref_images_b64: Optional[List[str]] = None) -> Dict:
        """
        ...
        Args:
            text: 用户自然语言描述
            ref_images_b64: 参考图片 base64 列表（可选）；提供后模型可识别主体特征，避免性别误推断
        """
        print(f"[VLM] 检测到自然语言输入，调用 VLM 进行智能转换...")

        valid_images = [b for b in (ref_images_b64 or []) if b]

        # 尝试顺序：1) 带图（若有）→ 2) 纯文本兜底（模型不支持视觉时自动降级）
        attempts_plan = []
        if valid_images:
            label = f"带参考图 x{len(valid_images)}" if len(valid_images) > 1 else "带参考图"
            attempts_plan.append((label, valid_images))
        attempts_plan.append(("纯文本", None))

        for mode_label, current_images in attempts_plan:
            last_error = None
            for attempt in range(VLM_MAX_RETRIES):
                try:
                    ...
                    # 有参考图时使用多模态格式，所有图片逐一传入
                    if current_images:
                        img_count = len(current_images)
                        if img_count == 1:
                            img_hint = "这是用户上传的参考图片，"
                        else:
                            img_hint = f"这是用户上传的 {img_count} 张参考图片，"
                        user_content = [
                            {
                                "type": "image_url",
                                "image_url": {
                        ...
                        ] + [
                            {
                                "type": "text",
                                "text": (
                                    f"{img_hint}请先观察图中主体的性别、外貌、服装等特征，"
                    ...
                    vision_err_keywords = ("does not support", "image", "vision", "multimodal",
                                           "400", "unsupported")
                    if current_images and any(k in err_str.lower() for k in vision_err_keywords):
    ...
    def validate_json(self, json_string: str, ref_image_b64: Optional[str] = None,
                      ref_images_b64: Optional[List[str]] = None) -> Dict:
        """
        验证/转换输入为标准提示词列表。
        ...
        Args:
            ...
            ref_image_b64: 单张参考图片 base64（兼容旧调用，优先使用 ref_images_b64）
            ref_images_b64: 多张参考图片 base64 列表（优先级高于 ref_image_b64）
        ...
        """
        # 合并两种传参方式，ref_images_b64 优先
        images_b64 = ref_images_b64 if ref_images_b64 else ([ref_image_b64] if ref_image_b64 else [])
        ...
        if self._is_natural_language(json_string):
            if self.client:
                return self._convert_natural_language(json_string, ref_images_b64=images_b64)
            else:
                return {
                    "valid": False,
...
```"""
    ...
    def optimize_single_prompt(self, prompt_text: str, ref_images_b64: Optional[List[str]] = None) -> str:
        """
        使用提示词优化器对单条 prompt 进行优化。

        Args:
            prompt_text: 原始提示词文本（中文或英文均可）
            ref_images_b64: 参考图片的 JPEG base64 列表（可选）；
        ...
        for mode_label, current_images in attempts_plan:
            try:
                # 构造 user 内容：有参考图时使用多模态格式，所有图片逐一传入
                if current_images:
                    img_count = len(current_images)
                    img_hint = f"这是用户上传的 {img_count} 张参考图片" if img_count > 1 else "这是用户上传的参考图片"
                    user_content = [
                        {
                            "type": "image_url",
                            "image_url": {
                    ...
                    ] + [
                        {
                            ...
                            "text": (
                                f"{img_hint}。\n\n"
                                "重要规则：\n"
                                "1. 不要将参考图中的当前状态（服装颜色、发型、体型等）转化为字面描述写入提示词，"
                                "这会导致生成结果不一致。\n"
                                "2. 必须用引用句式代替状态描述，例如：\n"
                                "   - 'consistent with the person in the reference image'\n"
                                "   - 'maintaining the same appearance as shown in the reference image'\n"
                                "   - 'with identical outfit/look to the reference image'\n"
                                f"{'   - 多图时：consistent with image 1 / as shown in image 2' if img_count > 1 else ''}\n\n"
    ...
    def optimize_prompts(
        ...
        Args:
            ...
            image_tensors: 用户上传的参考图片 tensor 列表（可选）；全部转为 base64 传入优化器
        ...
        # 将所有参考图转为 base64，所有并发任务共享同一份列表
        ref_images_b64: List[str] = []
        if image_tensors:
            print(f"[Optimizer] 正在将 {len(image_tensors)} 张参考图转换为 base64...")
            for t in image_tensors:
                b64 = self._tensor_to_base64(t)
                if b64:
                    ref_images_b64.append(b64)
            if ref_images_b64:
                print(f"[Optimizer] {len(ref_images_b64)} 张参考图已就绪，优化时将以这些图为准")
            else:
        ...
        def _optimize_one(idx: int, obj: Dict) -> tuple:
            ...
            if original:
                optimized = self.optimize_single_prompt(original, ref_images_b64 or None)
